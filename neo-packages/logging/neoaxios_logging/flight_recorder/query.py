...
Usage:
    python -m tools.flight_recorder.query [OPTIONS]
...
Examples:
  ...
  %(prog)s --event "pool_connection_*"

  # Trace specific worker
  %(prog)s --trace-worker gw00

  # Show stats grouped by worker
  %(prog)s --stats --group-by worker

  # Find slow operations
  %(prog)s --event "*_complete" --field "duration>1.0"

  # Timeline view with relative timing
  %(prog)s --format timeline --event "pool_*"
        """
    )
